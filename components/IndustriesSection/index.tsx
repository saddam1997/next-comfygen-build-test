...
function IndustriesServe({ Industries = defaultSliderData }: any) {

    const { heading, description, IndustriesServe } = Industries
    ...
    const next = () => {
        setCurrent((p) => {
            const max = Math.max(0, IndustriesServe.length - slidesToShow);
            return p >= max ? 0 : p + 1;
        });
    };
            ...
            <div className="mx-auto max-w-[1440px] xl:w-5/6 w-11/12 bg-[#F5F5F9] md:py-14 py-8 md:px-10 px-6 rounded-3xl">
                ...
                >
                    <div ref={trackRef} className="flex">
                        {IndustriesServe.map((item: any, i: any) => (
                            <div
                                key={i}
