...
export default function DesktopNav(props: any) {
  ...
  // 🧱 Toggle slideover menu (no direct DOM)
  const toggleSlideover = () => setMenuOpen((prev) => !prev);
  ...
  return (
    <header
      className={`fixed top-0 w-full bg-white z-50 max-w-[1600px] mx-auto ${isHeaderActive
        ? "bg-white"
        : "bg-white"
        }`}
    >
      ...
      <nav className="flex items-center justify-between px-4 py-0  md:px-0 relative 2xl:w-10/12 w-10/12 lg:w-11/12 mx-auto ">
        ...
        <div>
          ...
          >
            {menuOpen ? <MdMenuOpen size={26} /> : <MdMenu size={26} />}
          </div>
          <div
            className={
