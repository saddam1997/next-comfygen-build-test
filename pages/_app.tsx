...
const Talkchat = dynamic(() => import("./components/Talkchat"), { ssr: false });
...
const AppLayout = ({ Component, pageProps }: any) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      timeout = setTimeout(() => setIsVisible(window.scrollY > 50), 100);
    };
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  const excludePaths = ["/career", "/contact-us", "/portfolio"];
  const showGetInTouch = !excludePaths.some((path) =>
  ...
  return (
    <main className="w-full">
      <div className="max-w-[1600px] mx-auto overflow-y-auto">
        <Component {...pageProps} />
        ...
        {isVisible && (
          <button
            ...
            className="fixed bottom-10 right-10 bg-indigo-600 text-white rounded-full p-3"
          >
            ↑
          </button>
        )}
      ...
      {loaded && (
        <>
          ...
          <div>
            {isVisible && (
              <div className="fixed lg:bottom-10 bottom-[6rem] z-40 left-10">                     <button
