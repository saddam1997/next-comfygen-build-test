        ...
        <div className="md:space-y-10 space-y-6  w-full">
          ...
          <div className="w-full py-4 space-y-4">
            {faqData.map((elem, index) => {
              const { title, desc, decs, dec1 ,  dec2, faqList } = elem;

              return (
                <div key={index} className="bg-white border cursor-pointer w-full rounded-lg" onClick={() => setCurrentCount(currentCount === index ? false : index)}>
