...
interface CardListMinimalProps {
  techData: CardItemType[];
  heading?: string;
  subheading?: string;
}

const CardListMinimal: React.FC<CardListMinimalProps> = ({ techData = [], heading, subheading }) => {
  return (
    <section className='py-10 bg-white flex justify-center'>
      <div className='w-full max-w-7xl'>
        ...
        <div className='flex flex-col lg:flex-row justify-center items-start gap-10'>
          {techData.slice(0, 2).map((item, index) => (
            <div
              key={index}
              className='w-full max-w-md border-2 p-8 space-y-6 bg-white border-[#5556D1]/40 rounded-2xl shadow-md'
            >
              <h3 className='text-2xl text-[#212121] font-semibold text-start'>{item.title}</h3>
