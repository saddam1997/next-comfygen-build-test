...
export default function AppPricingTable({ Cost }) {
    return (
        <section className="w-full px-4 py-10 sm:px-6 lg:px-8 border-2 border-gray-900 rounded-md h-full flex items-center justify-center">
            ...
            <div className="w-full max-w-7xl space-y-3">
                ...
                <div className="flex flex-col justify-center text-center lg:w-5/6 mx-auto">
                    <HeadingTwo color="black" text={Cost?.heading} />

                    {Cost?.description && (
                        <p
                            className="pt-4 text-black"
                            dangerouslySetInnerHTML={{ __html: Cost.description }}
                        />
                    )}
                </div>
                    ...
                    <table className="hidden md:table w-full text-sm">
                        <thead>
                            <tr className="bg-[#1a3fa8]">
                                {["App Type", "Scope", "Timeline","Estimated Cost" ].map((heading) => (
                                    <th
                                        key={heading}
                        ...
                        <tbody>
                            {Cost?.rows?.map((row, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-5 text-gray-600 font-medium text-left">
                                        {row.Scope}
                                    </td>
                ...
                {/* {Cost?.points?.length > 0 && (
                    <div className="py-8 max-w-5xl mx-auto text-center">
                        <HeadingThree color="black" text={Cost?.subheading} />

                        <ul className="space-y-3 mt-4">
                            {Cost.points.map((point:any, index:any) => (
