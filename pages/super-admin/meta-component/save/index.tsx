...
// Updated interfaces to better handle nesting
...
// Utility function to get component data
...
// Updated Object Field Component with nested support
const ObjectField: React.FC<{
  properties: ObjectProp[];
  onChange: (properties: ObjectProp[]) => void;
  depth?: number;
}> = ({ properties, onChange, depth = 0 }) => {
  const addProperty = () => {
        ...
        <div key={index} className="space-y-2 border-l-2 border-gray-200 pl-4">
          ...
          {prop.type === 'OBJECT' && (
            <ObjectField
              ...
              onChange={(properties) => updateProperty(index, 'objectProps', properties)}
              depth={depth + 1}
            />
          )}
          ...
          {prop.type === 'ARRAYOFOBJECT' && (
            <ArrayObjectField
              ...
              onChange={(items) => updateProperty(index, 'arrayObjectProps', items)}
              depth={depth + 1}
            />
          )}
        </div>
...
const ArrayObjectField: React.FC<{
  ...
  return (
    <div className={`${depth > 0 ? 'ml-4' : ''} space-y-4`}>
...
const HeaderPropsForm: React.FC = () => {
  ...
  const handlePropChange = (index: number, field: keyof Prop, value: any) => {
