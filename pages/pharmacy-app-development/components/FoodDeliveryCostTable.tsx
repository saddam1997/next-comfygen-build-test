...
export default function AppPricingTable({ FoodDeliveryCost}) {
    return (
        <section className="w-full px-4 py-10 sm:px-6 lg:px-8 border-2 border-gray-900 rounded-md h-full flex items-center justify-center">
            ...
            <div className="w-full max-w-7xl space-y-3">
                ...
                <div className="flex flex-col justify-center text-center lg:w-5/6 mx-auto">
                    <HeadingTwo color="black" text={FoodDeliveryCost?.heading} />

                    {FoodDeliveryCost?.description && (
                        <p
                            className="pt-4 text-black"
                            dangerouslySetInnerHTML={{ __html: FoodDeliveryCost.description }}
                        />
                    )}
                </div>
                    ...
                    <table className="hidden md:table w-full text-sm">
                        <thead>
                            <tr className="bg-[#1a3fa8]">
                                {["Type of Pharmacy App", "Cost Range (USD)", "Timeline"].map((heading) => (
                                    <th
                                        key={heading}
                        ...
                        <tbody>
                            {FoodDeliveryCost?.rows?.map((row, i) => (
                                <tr
                                    key={row.appType}
