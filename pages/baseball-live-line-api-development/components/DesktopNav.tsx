...
export default function DesktopNav(props: any) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const newScrollState = window.scrollY > scrollLimit;
      if (!ticking) {
  ...
  const headerClasses = isScrolled
    ? "fixed top-0 w-full bg-white max-w-[1600px] z-50 "
    : "fixed top-0 w-full max-w-[1600px] mx-auto z-50 ";

  return (
    <div className={headerClasses}>
      <div className={ `flex gap-6 justify-end mx-10 ${isScrolled ? "text-black" :"text-white"}`}>
            ...
            <div className="block space-x-4 text-sm font-medium 2xl:space-x-4 lg:flex lg:items-center lg:space-y-0 lg:p-0">
              <div>
                <Link href="/" passHref={true}>
                  <span className={`relative block text-sm leading-none transition duration-300 border-b-2 border-transparent cursor-pointer py-9  ${isScrolled ? "hover:text-black/100 text-black/80":"text-white"} `}>
                    Home
                  </span>
                </Link>
              </div>
              ...
              >
                <button
                  ...
                  className={`inline-flex items-center py-8 text-sm border-b-2 border-transparent  ${isScrolled ? "hover:text-black/100 text-black/80" 
                    : "text-white"
                  }`}
                >
                  <span
              ...
              >
                <button
                  ...
                  className={`inline-flex items-center py-8 text-sm border-b-2 border-transparent  ${isScrolled ? "hover:text-black/100 text-black/80" :"text-white"}`}
                >
                  <span
                    className={
