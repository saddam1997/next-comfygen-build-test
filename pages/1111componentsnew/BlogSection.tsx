        ...
        <div className="grid gap-4 mx-auto lg:grid-cols-3 md:grid-cols-2 xl:gap-14 cursor-pointer mt-8">
          {props?.initialData && props?.initialData.length > 0
            ? props?.initialData.map((element: any, index: number) => (
              element ? (
                <div
                  key={index}  // ✅ key stays here since no fragment
                  className="bg-[#fff] border border-[#000000]/10 rounded-xl"
                >
                  <a href={element.link} className="block">
                    <div className="p-6 space-y-4 overflow-hidden text-left transition duration-150 group">
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          <button className="text-[#000] text-base rounded-full border border-[#000] py-1.5 px-5">
                            Categories
                          </button>
                        </div>
                        <div>
                          <button className="text-[#000] text-base flex items-center gap-2">
                            <FaCalendarAlt /> 4 Aug 2024
                          </button>
                        </div>
                      </div>
                      <div className="flex flex-col justify-between space-y-3">
                        ...
                        <div>
                          <button className="text-lg text-[#000000] flex items-center group-hover:text-[#5556D1]">
                            Learn More <MdOutlineNavigateNext size={24} />
                          </button>
                        </div>
                      </div>
                    </div>
