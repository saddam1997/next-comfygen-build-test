        ...
        <div className="md:space-y-10 space-y-6 w-full">
          <div>
            ...
            <p className="text-base text-[#212121] font-normal mt-2 w-full text-center">
              Some answers to the common queries during web and mobile app development.
            </p>
          </div>
            ...
            {faqData?.map((item, index) => {
              ...
              return (
                <div
                  ...
                  <div
                    className={`flex items-start gap-4 justify-between w-full px-4 py-4 rounded-tr-[14px] rounded-tl-[14px] text-base font-medium text-left md:items-center md:px-6 md:text-lg ${currentCount === index
                        ? "bg-[#5556D1]/10 text-[#212121]"
                        : "text-[#212121]"
                      }`}
                  >
                    <h3 className="w-full text-[14px] font-semibold md:text-lg">{itemTitle}</h3>
                    {currentCount === index ? (
                      <svg
                            ...
                            {card.items && card.items.length > 0 && (
                              <ul>
                                {card.items?.map((item, j) => (
                                  <li className="flex gap-1 items-start " key={j}> <MdStars className="mt-1" /> {item}</li>
                                ))}
                              </ul>
                            )}
                          </div>
